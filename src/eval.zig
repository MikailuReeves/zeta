...
pub fn evalExp(node: Node) EvalError!Value {
    switch (node.expr) {
        .literal => |value| {
            return value;
        },
        ...
        .unary => |u| {
            const operand_val = try evalExp(u.operand.*);
        ...
        .binary => |b| {
            const operand_left = try evalExp(b.left.*);
            const operand_right = try evalExp(b.right.*);
